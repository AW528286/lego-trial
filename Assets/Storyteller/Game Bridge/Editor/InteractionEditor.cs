...
namespace DaiMangou.GameBridgeEditor
{
    ...
    public class InteractionEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (showHelpMessage)
                EditorGUILayout.HelpBox("This will update the Interaction component with all the necessary data from the SceneData", MessageType.Info);
            ...
            if (interactionType.enumValueIndex == (int)InteractionType.Non_Linear)
                EditorGUILayout.HelpBox("In non-linear mode, you can use collision detection ot call BeginInteraction(InteractableCharacter) / BeginInteraction(gameObject) to interact with a chosen character.", MessageType.Info);
    ...
    selectedInteraction.KeywordFilters[i].Disabled ? ImageLibrary.PowerOffpro : ImageLibrary.PowerOnpro, "Enable / Disable this Keyword Filter"))
          ...
          if (dialogue.OverrideTextDisplayMethod)
            ...
            var showListOfGameFunctionsFoldoutArea = GUILayoutUtility.GetLastRect().ToLowerLeft(200, 20);
            showListOfGameFunctions = EditorGUI.Foldout(showListOfGameFunctionsFoldoutArea, showListOfGameFunctions, "Show list of game functions");

            if(showListOfGameFunctions)
            {
                EditorGUILayout.HelpBox(
                    "Basic Information : " +
