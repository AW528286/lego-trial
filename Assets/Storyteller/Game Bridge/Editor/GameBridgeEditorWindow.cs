        ...
        public void OnGUI()
        {
            ...
            switch (EditID)
            {
                ...
                case 1:
                    ...
                    GUI.Label(headerArea.ToCenterLeft(), "Scene : " + story.Scenes[CurrentStory.ActiveSceneIndex].SceneName);


                    if (SelectedSceneData.SceneID == -1)//temp
                        SelectedSceneData.SceneID = 0;// temp

                   
                        if(SelectedSceneData.UID == story.Scenes[CurrentStory.ActiveSceneIndex].UID)
                        {
                            if (SelectedSceneData.SceneID != CurrentStory.ActiveSceneIndex)
                                SelectedSceneData.SceneID = CurrentStory.ActiveSceneIndex;
                        }

                        if (SelectedSceneData.UID != story.Scenes[CurrentStory.ActiveSceneIndex].UID && SelectedSceneData.UID!= "")
                        {
                            ...
                            if (GUI.Button(ScreenRect.ToCenterBottom(0, 20), "Update"))
                                SelectedSceneData.UID = story.Scenes[CurrentStory.ActiveSceneIndex].UID;
                        ...
                        var characterCount = story.Scenes[CurrentStory.ActiveSceneIndex].Characters.Count;
                        Grid.BeginDynaicGuiGrid(characterSelectionArea, characterAreas, 10, 20, 120, 60, story.Scenes[CurrentStory.ActiveSceneIndex].Characters.Count);
                        ...
                        if (story.Scenes[CurrentStory.ActiveSceneIndex].UID != CachedUID)
                        {
                            story.Scenes[CurrentStory.ActiveSceneIndex].Characters.All(p => p.Pushable == true);
                            CachedUID = story.Scenes[CurrentStory.ActiveSceneIndex].UID;
                        }
                        ...
                        for (int i = 0; i < characterCount; i++)
                        {
                            ...
                            var targetCharacter = story.Scenes[CurrentStory.ActiveSceneIndex].Characters[i];
                        ...
                        // tell storyteller to do a cleanup check

                        var cleanupWindow = GetWindow<CleanupEditor>();
                        cleanupWindow.FindAllBrokenNodes();
                        if (cleanupWindow.BrokendNodes.Count > 0)
                        {
                            for (int i = 0; i < cleanupWindow.BrokendNodes.Count; i++)
                                Debug.LogWarning(cleanupWindow.BrokendNodes[i].Name + " is a broken node. the data push will be halted.use the cleanup editor to remove broken nodes");

                            return;
                        }

                        cleanupWindow.FindAllIllegalConnections();
                        if (cleanupWindow.NodesWithIllegalConnections.Count > 0)
                        {
                            for (int i = 0; i < cleanupWindow.NodesWithIllegalConnections.Count; i++)
                                Debug.LogWarning(cleanupWindow.NodesWithIllegalConnections[i].Name + " is has a illegal connection. the data push will be halted.use the cleanup editor to remove illegal connections");

                            return;
                        }

                         cleanupWindow.FindAllUnusedNodes();
                         if (cleanupWindow.UnusedNodes.Count > 0)
                         {
                             for (int i = 0; i < cleanupWindow.UnusedNodes.Count; i++)
                                 Debug.LogWarning(cleanupWindow.UnusedNodes[i].Name + " is unused. the data push will be halted.use the cleanup editor to remove unused nodes");
                         }

                        if (cleanupWindow.BrokendNodes.Count == 0 && cleanupWindow.NodesWithIllegalConnections.Count == 0 && cleanupWindow.UnusedNodes.Count == 0)
                            cleanupWindow.Close();
                       /* cleanupWindow.FindGhostNodes();
                        if (cleanupWindow.GhostNodes.Count > 0)
                        {
                            for (int i = 0; i < cleanupWindow.GhostNodes.Count; i++)
                                Debug.LogWarning(cleanupWindow.GhostNodes[i].Name + " is a ghost node. the data push will be halted.use the cleanup editor to remove broken nodes");
                        }*/
                        ...
                        SelectedSceneData.UID = story.Scenes[CurrentStory.ActiveSceneIndex].UID;
                            ...
                            /*   var routes = story.Scenes[CurrentStory.ActiveSceneIndex].NodeElements.Where(r => r.GetType() == typeof(RouteNode)).ToList();
                               ...
                               foreach (var ch in story.Scenes[CurrentStory.ActiveSceneIndex].Characters)
                               {
                            ...
                            foreach (var el in story.Scenes[CurrentStory.ActiveSceneIndex].NodeElements)
                            {
                            ...
                            for (var i = 0; i < SelectedSceneData.FullCharacterDialogueSet.Count; i++)
                            {
                                ...
                                var matchingStoryElement = CurrentStory.ActiveStory.Scenes[CurrentStory.ActiveSceneIndex]
